...
<a id="2-자동-생성되는-최대-7개-대시보드-싱글톤-페이지"></a>
...
| **AI 가공도 다 받기** (재원 중, 풍부하게) | `use_ai_features=on` + 4개 dashboard 모두 `on` (기본값) |
...
다른 자녀를 백업하려면 `Run workflow` 화면의 **`child_name`** 칸에 자녀 이름을 입력하세요 (예: `유주`). 자세한 가이드는 [👨‍👩‍👧‍👦 (고급) 자녀가 여러 명이거나 AI 편지를 끄고 싶을 때](#advanced-multichild-and-ai-off) 섹션을 참고하세요.
