...
>
> **자녀 선택도 같은 방식** — 백업할 아이를 고정하는 secret은 **`KIDSNOTE_CHILD_NAME`**. 값은 글자 수 자유 (substring 부분 일치) — `우하린`/`하린` 다 OK, `정에스더`/`스더` 다 OK. 자녀가 1명이어도 꼭 등록하세요 — API 자동 선택에 의존하지 않고 동생이 생겨도 안전합니다. 자녀가 여러 명인 경우에는 [👨‍👩‍👧‍👦 자녀가 여러 명이거나 AI 편지를 끄고 싶을 때](#advanced-multichild-and-ai-off) 섹션 참고.
