...
> 🤖 **AI 가공은 7단계 secret에서 on/off로 선택** — `AI_FEATURES=off`면 알림장·사진·댓글·식단·앨범·공지 + 통계 대시보드 2종만 깔끔하게 (1년치 1-3시간). `AI_FEATURES=on`이면 자녀 일기/부모 편지/LLM 대시보드 4종까지 추가 (1년치 5-15시간, cron 자동 재개). AI 가공은 무료 로컬 모델로 만들어지니 가끔 어색한 부분은 노션에서 직접 수정하거나 [B 섹션](#b-ai-가공을-안-쓰고-단순-백업--통계만-원할-때)에서 값을 `off`로 바꾸면 됩니다.
...
- [⭐ 시작하기 — 8단계](#시작하기)
  ...
  - [7단계. GitHub Secrets](#7단계-github에-5개-비밀-값-등록하기)
...
각 알림장은 **노션 페이지 한 장**으로 백업됩니다 — 원본 본문 + 사진(원본 화질·GPS 자동 제거) + 식단 사진 + 댓글 + 날씨. 자녀 일기/부모 편지 같은 AI 가공 콘텐츠는 7단계에서 `AI_FEATURES` secret 값으로 `on`/`off` 선택 (자세히는 [B 섹션](#b-ai-가공을-안-쓰고-단순-백업--통계만-원할-때)).
...
<a id="7단계-github에-5개-비밀-값-등록하기"></a>
...
| `AI_FEATURES` | **`on`** (AI 가공 켜기, 1년치 5-15시간) 또는 **`off`** (AI 가공 끄기, 1년치 1-3시간) |
...
자녀 #2용 fork에서 [**7단계**](#7단계-github에-5개-비밀-값-등록하기) 그대로 진행:
...
**`AI_FEATURES=off`** 로 설정하면 AI 가공 없이 단순 백업 + 통계 대시보드 2개(📊 통계 / 🥗 영양)만 생성됩니다 (1년치 1-3시간). 졸업·휴원 후 백업이나 사생활 보수적인 사용자에게 가장 잘 맞아요.

<a id="ai-한-방-스위치-aifeatures"></a>

### 🎚️ AI 한 방 스위치 — `AI_FEATURES` secret

셋업 7단계에서 이미 `AI_FEATURES` secret을 등록하셨을 거예요. **값만 바꾸면** 수동 실행과 cron 자동 실행 둘 다 동시에 적용됩니다.
...
```
AI_FEATURES = off  →  AI 끔 (1년치 1-3시간, 깔끔한 키핑 백업)
AI_FEATURES = on   →  AI 켬 (1년치 5-15시간, 자녀 일기/부모 편지/LLM 대시보드 4종 추가)
```
