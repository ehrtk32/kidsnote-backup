...
>
> **자녀 선택도 같은 방식** — 자녀가 2명 이상일 때 백업할 아이를 고르는 secret은 **`KIDSNOTE_CHILD_NAME`** (값: 자녀 이름 일부, 예 `하린`). 한 번 등록하면 cron 자동 실행에도 동일하게 적용됩니다. 자세히는 [👨‍👩‍👧‍👦 자녀가 여러 명이거나 AI 편지를 끄고 싶을 때](#advanced-multichild-and-ai-off) 섹션 참고.
...
다른 자녀를 백업하려면 fork repo **`Settings` → `Secrets and variables` → `Actions`** 에서 secret **`KIDSNOTE_CHILD_NAME`** 을 추가하고 백업할 자녀 이름의 일부(예: `유주`)를 입력하세요. cron 자동 실행에도 같이 적용됩니다. 자세한 가이드는 [👨‍👩‍👧‍👦 (고급) 자녀가 여러 명이거나 AI 편지를 끄고 싶을 때](#advanced-multichild-and-ai-off) 섹션을 참고하세요.
...
| **다른 자녀가 백업됨** (둘째인데 첫째가 백업됨) | 자녀가 둘 이상이면 API상 첫 번째 자녀가 default. secret `KIDSNOTE_CHILD_NAME` 누락 | repo Settings에서 secret **`KIDSNOTE_CHILD_NAME`** 추가, 값에 자녀 이름 일부(예: `유주`) 입력. 자세히는 [(고급) 자녀가 여러 명이거나 AI 편지를 끄고 싶을 때](#advanced-multichild-and-ai-off) 섹션 |
