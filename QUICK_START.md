...
> 🤖 **AI 가공은 기본값 `off`** — 그냥 `Run workflow` 클릭하면 키즈노트 원본 + 통계 대시보드 3개만 빠르게 백업됩니다 (1년치 1~3시간). AI가 만드는 자녀 일기/부모 편지/LLM 대시보드 4종을 받고 싶으면 `use_ai_features=on`으로 변경 (느려짐 — 1년치 20~50시간).
...
- 1년치(300~400개) 첫 백업 소요 시간:
  - **AI 끔** (기본값, `use_ai_features=off`) → **1~3시간** (사진 업로드 시간 대부분, Ollama 다운로드 + LLM 호출 없음)
  - **AI 켬** (`use_ai_features=on`) → **20~50시간** (llama3.1:8b 모델로 알림장당 LLM 호출 3회 + 4개 대시보드 생성 포함)
