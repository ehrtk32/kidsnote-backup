...
> 🤖 **AI 가공이 마음에 안 들면** `Run workflow` 화면에서 `use_ai_features=false`로 한 번에 끌 수 있어요. 키즈노트 원본 + 통계 대시보드 3개만 깔끔하게 백업됩니다.
...
4. 15~25분 후 노션 DB 확인:
   - 알림장 3건 + 사진 OK
   - 📊 통계 / 📅 작년추억 / 🥗 영양 / 📖 성장스토리 / 🌟 마일스톤 / 🌱 관심사 / 💌 선생님께 — 최대 7개 대시보드 자동 생성
...
- 1년치(300~400개) 첫 백업 소요 시간:
  - **AI 끔** (`use_ai_features=false`) → **1~3시간** (사진 업로드 시간 대부분, Ollama 다운로드 + LLM 호출 없음)
  - **AI 켬** (기본값) → **20~50시간** (qwen2.5:14b 모델로 알림장당 LLM 호출 3회 + 4개 대시보드 생성 포함)
